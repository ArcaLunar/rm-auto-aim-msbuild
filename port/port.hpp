...
template <typename Tp = SentryVisionRecvMsg> // sentry uses a different protocol
class SerialPort : public std::enable_shared_from_this<SerialPort<Tp>> {
  ...
  public:
    ...
    bool send_message(const VisionPLCSendMsg &payload) {
        ...
        if (options.port.inspect_data) {
            spdlog::info(
                "Sending data: {:X:n}",
                spdlog::to_hex(this->send_buffer.data(), this->send_buffer.data() + kSendMsgSize)
            );
        }
    ...
    void process_raw() {
        ...
        while (true) {
            ...
            auto [raw, success] = this->raw_cbuffer.pop();
  ...
  private:
    ...
    CircularBuffer<Tp> recv_cbuffer;
  ...
  protected:
     ...
     */
    bool verify(const RawMessage &msg, size_t i, size_t j) {
        if (j <= i || j - i < kRecvMsgSize) {
            spdlog::error("Invalid message size: {} ~ {}", i, j);
            return false;
        }
        // Verify that the message follows the protocol format
        if (msg[i] != cfg.startbyte || msg[j - 1] != cfg.endbyte) {
            spdlog::error("testing {:X:n}", spdlog::to_hex(msg.data() + i, msg.data() + j));
            spdlog::error(
                "Invalid message format: start byte or end byte mismatch, start={} end={}", msg[i], msg[j - 1]
            );
            return false;
        }
