...
// ========================================================
// implementation of PoseCVT
// ========================================================

PoseCVT::PoseCVT(std::string cfg_path) {

    try {
        if (options.tf.init)
            spdlog::info("PoseCVT init");
        ...
        auto F = [&](const std::string &_s, cv::Mat &_res) {
            ...
            spdlog::info("{} has been initialized", _s);
        };
        ...
        spdlog::error("failed to init pose transformer: {}", e.what());
    }
}

Armor3d PoseCVT::solve_absolute(const AnnotatedArmorInfo &info) {
    Armor3d result;
    ...
    if (options.tf.debug_cvt)
        spdlog::info("estimated bullet flying time: {}", result.bullet_flying_time);

    return result;
}

cv::Mat PoseCVT::from_armor_to_camera(const pose_under_camera_coord &relative) {
...
cv::Mat PoseCVT::from_camera_to_imu() {
...
cv::Mat PoseCVT::from_imu_to_base(const IMUInfo &imu) {
...
cv::Mat PoseCVT::from_base_to_barrel() {
